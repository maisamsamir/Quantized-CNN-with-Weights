...
def train_and_evaluate(model, num_epochs=1):
...
def save_model(model, filename='fashion_mnist_cnn.pth'):
    torch.save(model.state_dict(), filename)
    print(f"Model weights saved to {filename}")


def load_model(model, filename='fashion_mnist_cnn.pth'):
...
def print_weights_as_numpy(model):
    state_dict = model.state_dict()
    for name, param in state_dict.items():
        print(f"Layer: {name}")
        print(f"Shape: {param.shape}")
...
print_all_weights(model)  
print_layer_weights(model, 'layer1.0.weight')  
print_weights_as_numpy(model)
def imshow(img):
    img = img / 2 + 0.5
    npimg = img.numpy()
